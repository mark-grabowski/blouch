...
transformed parameters {
  ...
  matrix[N,Z_adaptive] pred_X = calc_dmX(a,T_term,X_true); //Given measurement error in X variable, uncomment this nested statement
...
model {
  ...
  for(i in 1:(Z_adaptive)){//Given measurement error in X variable, uncomment this nested statement
    target += normal_lpdf(X_true[,i]|0,1);
    target += normal_lpdf(X_obs[,i]|X_true[,i],X_error[,i]);
  }

  target += multi_normal_cholesky_lpdf(Y_true | mu , L_v);
  target += normal_lpdf(Y_obs | Y_true, Y_error);
}
