//mvOU model using Mitov et al. (2020) approach
//Single regime version, free H and free Sigma
//Most up to date version - 09/20/2025
...
functions {
  ...
  matrix buld_upper_tri_T(int n_traits, vector T_upper_tri){
    matrix[n_traits,n_traits] T_ut = identity_matrix(n_traits); //Start with identity matrix for Q
    int pos = 1;
    for(j in 1:(n_traits-1)){ //Controls columns
      for(i in (j+1):n_traits){ //Controls rows
        T_ut[j,i] = T_upper_tri[pos];
        pos = pos + 1;
        }
      }
    return T_ut;
...
model {
  ...
  sigma_Sigma ~ lognormal(1, 0.25); // Centered around true sigma_Sigma values
