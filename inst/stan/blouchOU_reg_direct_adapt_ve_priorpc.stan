...
generated quantities {
  ...
  matrix[n_reg,Z_direct+Z_adaptive] beta_sim;
  ...
  V = calc_V(a,sigma2_y,ta,tij,tja,T_term,beta_sim[,(Z_direct+1):(Z_direct+Z_adaptive)],sigma2_x,Z_adaptive,n_reg);
  L_v = cholesky_decompose(V);
  optima_matrix = calc_optima_matrix(N, n_reg, a, t_beginning, t_end, times, reg_match, nodes);
  pred_X = calc_mixed_dmX(a,T_term,X_sim,Z_direct,Z_adaptive);
  for(i in 1:N){
    mu[i] = optima_matrix[i,]*optima_sim+pred_X[i,]*beta_sim[reg_tips[i],]';
    }
