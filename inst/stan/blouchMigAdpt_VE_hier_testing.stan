...
//Implementing hierarchical priors on all three
...
transformed parameters {
  ...
  //log(sd_theta[i]) ~ normal(mu_log_sd_theta, sigma_log_sd_theta) - Original relationship

  vector<lower=0.00001>[N] es;      // Population-specific es
  vector<lower=0.00001>[N] sd_theta; // Population-specific standard deviations for theta deviations
  vector<lower=0.00001>[N] sd_e;     // Population-specific standard deviations for residual deviations
...
generated quantities {
  ...
  for(i in 1:N){
    theta[i] = alpha[reg_assign[i]] + x_true[i] * beta[reg_assign[i]];
  }
  for (n in 1:N) {
    var_z[n] = Sigma[n, n];
