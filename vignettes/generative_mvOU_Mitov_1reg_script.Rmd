...
# Simulated "true" values from your setup
true_vals <- list(
  sigma_P = c(sqrt(0.1), sqrt(0.25)),  # From Sigma = diag(sigma) %*% Omega %*% diag(sigma)
...
plot_prior <- function(samples, true_value, title, xlab = "Value", xlim) {
  ...
  ggplot(df, aes(x = value)) +
    ...
    geom_vline(xintercept = true_value, color = "red", size = 1) +
    ggtitle(title) +
    xlab(xlab) +
    xlim(xlim) +
    theme_minimal()
...
p7 <- plot_prior(theta[, 1], true_vals$theta[1], "Prior: theta[1]", xlim = c(-5,15))
p8 <- plot_prior(theta[, 2], true_vals$theta[2], "Prior: theta[2]", xlim = c(-5,15))
...
```{r}
init = function() list(
  #L_Omega_Sigma = diag(2),
  sigma_Sigma = c(1.0, 1.0),
  rho = 0.1)
...
print(stan_fit,
      ...
      pars = c("H_mats", "Sigma_mats", "sigma_Sigma","theta_mats",
               #"Omega_Sigma",
               #"rho",
               #"y_true",
               "Sigma_eigenvalues",
