...
H_mat_reg1<-matrix(c(2,0.5, #H Matrix - 
...
Sigma_reg1 <- diag(sigma_Sigma) %*% Omega_Sigma %*% diag(sigma_Sigma)
...
calc_current_node_state<-function(current_node, parent_node, parent_state, branch_length, regime_id_for_branch, H_mats, Sigmas, theta_mat){ #
  ...
  Sigma_mat <- Sigmas[[regime_id_for_branch]]
...
# Also scale the per-trait measurement error
y_error_sd <- 0.01
dat$y_error <- matrix(rep(y_error_sd / trait_sds, each = N), nrow = N, byrow = FALSE)
...
Omega_Sigma_list <- rlkjcorr(n_sims, K = n_traits, eta = 4)
...
sigma_Sigma <- cbind(rlnorm(n=n_sims,log(0.7),0.25),rlnorm(n=n_sims,log(0.7),0.25)) #Mean around true values - 
