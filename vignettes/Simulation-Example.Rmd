...
We can also print out and compare a few of the parameter estimates with the values we set earlier.
...
Now lets plot the covariance as a function of distance from the tips - this gives an idea of the decay of covariance in the OU process. We will use *Blouch*'s R helper function, calc_multiadaptive_cov_plot.R to make these plots:
...
optima.sims<-rnorm(100,2.88,1)
beta.sims<-rnorm(100, 0.16,0.25)
...
Lets do some model comparison using Pareto-Smoothed Importance Sampling (PSIS) from the R Package *loo* (Vehtari et al. 2023). loo estimates leave-one-out cross validation for Bayesian analyses. Here we are looking for Pareto k values below ~0.7, which suggest the results are accurate. The one species that has high k values for both models is *Tarsius bancanus*, the only tarsier in the analysis and a lineage that has evolved independently for a long period of time (i.e. has a long single branch on the phylogeny). This leads to it being hard to predict based on other species in the tree. PSIS also provides a way to look at outliers in our analysis.
...
```{r Posterior_pc}
########################################################################################################
fit.reg.adapt.mlm.ve.postpc<-rstan::sampling(object = blouch:::stanmodels$blouchOU_reg_adapt_mlm_ve_postpc,data = dat,chains = 1,cores=1,iter =4000)#, algorithm=c("Fixed_param"))
...
Posterior predictive checks suggest the model is reasonably reproducing the true values - no pattern is evident in the residuals.

### Prior predictive checks
Now lets do the same for the non-multilevel model.
```{r Prior_pc2}
...
Again, prior predictive checks show a generally reasonable fit between the data and data generated from the priors, though a few larger true values suggest using larger scale on some priors may be warranted
