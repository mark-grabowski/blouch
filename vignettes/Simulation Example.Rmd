...
fit.reg.adapt.mlm.ve<- rstan::sampling(blouch:::stanmodels$blouchOU_reg_adapt_mlm_ve,data = dat,chains = 2,cores=2,iter =2000)
```

```{r Plot_results_MLM_VE_Model}
print(fit.reg.adapt.mlm.ve,pars = c("hl","vy","optima_bar","beta_bar","Rho","sigma","optima","beta","beta_e"))
rethinking::precis(fit.reg.adapt.mlm.ve,depth=2,pars = c("hl","vy","optima_bar","beta_bar","Rho","sigma","optima","beta","beta_e") )#For use with rethinking package
...
```
These results suggest the two models are indistinguishable - 
...
```{r Prior_pc}
fit.reg.adapt.mlm.ve.priorpc<- rstan::sampling(object = blouch:::stanmodels$blouch,data = dat,chains = 2,cores=2,iter =2000, algorithm=c("Fixed_param"))
...
priorpc.plot<-ggplot2::ggplot()+  
  ...
  ggplot2::geom_abline(intercept=0,slope=1,alpha=0.05)+ #Prior
