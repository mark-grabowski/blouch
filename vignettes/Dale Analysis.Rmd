...
dalech_dat <- tibble(
    ...
    pop_size = dalech.data$Size, #What is this exactly? Population size of the visitors?
    pop_n = dalech.data$n, #Number of visits?
    GA_F = dalech.data$GA_F, #Field
    GA_F_SE = dalech.data$GASE_F,
    GA_F_N = dalech.data$N,
    GA_G = dalech.data$GA, #Greenhouse
...
e.g.
matrix[N_pops, N_pops] diag_sd_theta = diag_matrix(sd_theta); Individual sd_thetas per population
matrix[N_pops, N_pops] diag_sd_e = diag_matrix(sd_e); Individual sd_e per population
...
# Regression plots - dashed line is standard regression for greenhouse, dotted line is for field
```{r}
########################################################################################################
mypal <- ggsci::pal_aaas("default", alpha = 1)(3)

df<-data.frame(GA_F = dalech_dat$GA_F,GA_G = dalech_dat$GA_G,slp = dalech_dat$slp)
standard_reg_G<-lm(df$GA_G~df$slp)
standard_reg_F<-lm(df$GA_F~df$slp)

reg_df_G<-data.frame(standard_reg_G$coefficients)
reg_df_F<-data.frame(standard_reg_F$coefficients)
...
regression.plot<-ggplot2::ggplot()+  
  ...
  ggplot2::geom_abline(data=reg_df_G, ggplot2::aes(intercept=reg_df_G[1,1], slope=reg_df_G[2,1]),alpha=0.5 , linetype = 2)+ #Priors
  ggplot2::geom_abline(data=reg_df_F, ggplot2::aes(intercept=reg_df_F[1,1], slope=reg_df_F[2,1]),alpha=0.5 , linetype = 3)+ #Priors
...
regression.plot<-ggplot2::ggplot()+  
  ggplot2::geom_point(data=df,ggplot2::aes(y=GA_G,x=slp,color=mypal[1]))+
  ...
  ggplot2::ylab("GA (mm)") + ggplot2::xlab("SLP")+
  ggplot2::scale_color_manual(name="Sample Composition",values=mypal[2],labels=c('Greenhouse'))
...
dalech.data<-dalech.data[-c(3,18),] #Remove populations missing from M info

M_mat_raw_subset<-M_mat_raw[matching_M_pops,matching_M_pops] #Reorder by new index

N_pops <- nrow(dalech.data) #Get reduced number of populations
