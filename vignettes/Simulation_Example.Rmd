...
Lets check out our simulated data with reasonable values for the priors shown in light grey lines
```{r}
############################################################################################################
...
fit.reg.adapt.mlm.ve<- rstan::sampling(stanmodels$blouchOU_reg_adapt_mlm_ve,data = dat,chains = 2,cores=2,iter =4000,show_messages=FALSE)
print(fit.reg.adapt.mlm.ve,pars = c("hl","vy","optima_bar","beta_bar","Rho","sigma","optima","beta","beta_e"))
#plot(precis(fit.reg.adapt.mlm.ve,depth=3,pars = c("hl","vy","optima_bar","beta_bar","Rho","sigma","optima","beta","beta_e"))) #For use with rethinking package
...
fit.reg.adapt.ve<- rstan::sampling(stanmodels$blouchOU_reg_adapt_ve,data = dat,chains = 2,cores=2,iter =4000,show_messages=FALSE)
print(fit.reg.adapt.ve,pars = c("hl","vy","optima","beta","beta_e"))
#plot(precis(fit.reg.adapt.ve,depth=3,pars = c("hl","vy","optima","beta","beta_e")))#For use with rethinking package
post.ve<-rstan::extract(fit.reg.adapt.ve)#Extract posterior distribution 

```

Great. Now lots plot the prior vs. the posterior for the most important paramters. Lets use the Mulilevel Multi-optima Adaptive Model with Varying Effects - the other models posterior looks quite similar.
...
hl.post<-data.frame(post.mlm.ve$hl) #Using this model's posterior
...
#par(mar=c(3,3,0.25,0.25)) #Uncomment if want to write to PDF
...
Now lets compare our two models using Bayes Factors. Here we use the bridgesampling R package. Looking below, we can read the results as the data is X times more likely under a model that assumes a direct effect model rather than af adaptive model.
```{r}
########################################################################################################
#Bayes Factors
...
fit.reg.adapt.mlm.ve.priorpc<- rstan::sampling(object = stanmodels:,data = dat,chains = 2,cores=2,iter =2000, algorithm=c("Fixed_param"))
