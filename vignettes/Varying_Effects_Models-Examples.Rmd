...
Here I walk you through the varying effects models of Blouch. By varying effects I mean varying intercepts and slopes, which to my knowledge has not been done previously for OU models in the adaptation-inertia framework. We will run three different models below - a multilevel model which pools information across the regimes, a non-centered version of the same model to aid in exploring the posterior, and a non-multilevel version of the varying effects model. In all cases the number of iterations is small (400) and we are only running one chain - because this is just an example vignette and these models can take some time to run. In a real analysis you would want to run at least 1000 iterations and at least two chains.
...
```{r results='hide'}
fit.reg.adapt.mlm.ve<- rstan::sampling(object = blouch:::stanmodels$blouchOU_reg_adapt_mlm_ve,data = dat,chains = 1,iter =400,cores=1)
```
...
```{r results='hide'}
fit.reg.adapt.mlm.ve.nc<- rstan::sampling(object = blouch:::stanmodels$blouchOU_reg_adapt_mlm_ve_nc,data = dat,chains = 1,iter =400,cores=1)
```
...
```{r results='hide'}
fit.reg.adapt.ve<- rstan::sampling(object = blouch:::stanmodels$blouchOU_reg_adapt_ve,data = dat,chains = 1,iter =400,cores=1)
```
...
```{r results='hide'}
fit.reg.direct.adapt.mlm.ve<- rstan::sampling(object = blouch:::stanmodels$blouchOU_reg_direct_adapt_mlm_ve,data = dat,chains = 1,iter =400,cores=1)
...
```{r results='hide'}
fit.reg.direct.adapt.mlm.ve.nc<- rstan::sampling(object = blouch:::stanmodels$blouchOU_reg_direct_adapt_mlm_ve_nc,data = dat,chains = 1,iter =400,cores=1)
```
...
```{r results='hide'}
fit.reg.direct.adapt.ve<- rstan::sampling(object = blouch:::stanmodels$blouchOU_reg_direct_adapt_ve,data = dat,chains = 1,iter =400,cores=1)
```
