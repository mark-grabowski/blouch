...
for(i in 1:N_pops){
   m_i = sum(M_mat[i, -i]) # Total immigration rate into pop i not including diagonal
...
#Setup data for Stan
GA<-dalech_dat$GA_G #GA for Greenhouse only
GA_SE<-c(dalech_dat$GA_G_SE) #SE in greenhouse only
...
Define priors
```{r}
alpha_prior<-c(mean(GA),1) # Stan model expects 'alpha_prior' name (unless changed in Stan)
...
obs_df <- data.frame(GA_G = dalech_dat$GA_G, slp = dalech_dat$slp)
...
regression.plot <- ggplot2::ggplot() +
  ...
  ggplot2::scale_linetype_manual(
    name = "Model Type",
    values = c("OLS Regression" = "dotted", "M-S Model" = "solid")
  )
...
```
##############################################################################################################################
Create Stan dat file - MS model - fixed es model, starting at 0.01
##############################################################################################################################
...
n_chains<-2
#MigrationSelection Model
stan_fit_MS_es_001 <- rstan::sampling(
    object = stan_model,
    data = dat,
    chains = n_chains,             # Use the specified number of chains
    cores = min(n_chains, parallel::detectCores()), # Use available cores up to n_chains
...
post_MS_es_001<-rstan::extract(stan_fit_MS_es_001)
```

Pairs plot for estimated parameters
```{r, include=FALSE}

pairs(stan_fit_MS_es_001, 
            pars = c("alpha", "beta", 
               "A[1,1]","A[2,2]",
               "sd_theta",
               "sd_e"
...
n_chains<-2
#MigrationSelection Model
stan_fit_MS_es_01 <- rstan::sampling(
    object = stan_model,
    data = dat,
    chains = n_chains,             # Use the specified number of chains
    cores = min(n_chains, parallel::detectCores()), # Use available cores up to n_chains
...
n_chains<-2
#MigrationSelection Model
stan_fit_MS_es_1 <- rstan::sampling(
    object = stan_model,
    data = dat,
    chains = n_chains,             # Use the specified number of chains
    cores = min(n_chains, parallel::detectCores()), # Use available cores up to n_chains
...
post_MS_es_1<-rstan::extract(stan_fit_MS_es_1)
```

Pairs plot for estimated parameters
```{r, include=FALSE}

pairs(stan_fit_MS_es_1, 
            pars = c("alpha", "beta", 
               "A[1,1]","A[2,2]",
               "sd_theta",
               "sd_e"
...
n_chains<-2
#MigrationSelection Model
stan_fit_MS_es_216 <- rstan::sampling(
    object = stan_model,
    data = dat,
    chains = n_chains,             # Use the specified number of chains
    cores = min(n_chains, parallel::detectCores()), # Use available cores up to n_chains
...
post_MS_es_216<-rstan::extract(stan_fit_MS_es_216)
```

Pairs plot for estimated parameters
```{r, include=FALSE}

pairs(stan_fit_MS_es_216, 
            pars = c("alpha", "beta", 
               "A[1,1]","A[2,2]",
               "sd_theta",
               "sd_e"
...
##############################################################################################################################
Make Combined Regression Plot for all es runs
##############################################################################################################################
...
posterior_params_MS<- stan_fit_MS %>%
    spread_draws(alpha,beta) %>%
    mutate(model = "es = 2.16")

posterior_params_216<- stan_fit_MS_es_216 %>%
    spread_draws(alpha,beta) %>%
    mutate(model = "es = 2.16")


posterior_params_MS_001 <- stan_fit_MS_es_001 %>%
    spread_draws(alpha,beta) %>%
    mutate(model = "es = 0.01")

posterior_params_MS_01 <- stan_fit_MS_es_01 %>%
    spread_draws(alpha,beta) %>%
    mutate(model = "es = 0.1")


posterior_params_MS_1 <- stan_fit_MS_es_1 %>%
    spread_draws(alpha,beta) %>%
    mutate(model = "es = 1.0")

# Combine the tidy draws into a single data frame
combined_draws <- bind_rows(posterior_params_216, posterior_params_MS_001, posterior_params_MS_01, posterior_params_MS_1)
...
regression.plot <- ggplot2::ggplot() +
  ...
  ggplot2::scale_linetype_manual(
    name = "es Parameter",
    values = c("es = 2.16" = "solid","es = 0.01" = "dotted", "es = 0.1" = "dotdash","es = 1.0" = "twodash" )
  )
